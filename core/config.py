...
class SystemConfig(BaseModel):
    ...
    # Network & Server
    host: str = Field(
        default_factory=lambda: os.getenv("CYBER_SWARM_HOST", "0.0.0.0"),  # nosec B104
        description="Server host (0.0.0.0 allows multi-device cockpit access)"
    )
    port: int = Field(
        default_factory=lambda: int(os.getenv("CYBER_SWARM_PORT", "8420")),
